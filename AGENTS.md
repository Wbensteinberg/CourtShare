...
| Route | Method | Auth | Purpose |
| --- | --- | --- | --- |
| **`/api/create-checkout-session`** | `POST` | **Bearer** | Validates slot, conflicts, advance rules, **server-side price** from `courts/{courtId}`; rate-limited; Stripe Checkout with **manual capture** metadata. Body: **`courtId`**, **`date`**, **`time`**, **`durationMinutes`**, optional **`courtNumber`**. Uses shared conflict checks so confirmed and still-actionable pending requests block overlapping slots for the same court/court number. |
...
User-facing payment copy should keep exact semantics while using friendly phrases such as `Payment is complete`, `card authorization has been released`, or `refund has been issued`.
