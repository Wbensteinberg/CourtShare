...
- **Authenticated:** Top-right Messages control → `/messages` with unread badge count from conversations where the user is in `unreadBy`; profile/avatar menu expands with Upcoming Bookings, Messages, Host Dashboard, Profile, and log out.
...
`src/lib/` contains shared logic and integrations. `firebase.ts` initializes client Firebase and mock-mode detection, `firebase-admin.ts` initializes Admin SDK for API routes, `AuthContext.tsx` provides auth/profile state, `mockData.ts` stores seeded local mock users/courts/bookings/messages, `bookingDates.ts` centralizes booking date parsing/filtering/sorting and pending-request expiry helpers, `bookingConflicts.ts` centralizes overlap detection for confirmed and actionable pending requests, `bookingCreation.ts` finalizes checkout sessions into booking records, `conversations.ts` creates booking request conversations, `stripeBookingPayments.ts` releases uncaptured authorizations or refunds captured payments, `geolocation.ts` contains location helpers, `waivers.ts` contains waiver versions/content, `email.ts` sends transactional email, `reviewVisibility.ts` centralizes public review visibility rules, `theme.ts` mirrors key theme colors for TypeScript, and `utils.ts` contains generic utility helpers.
