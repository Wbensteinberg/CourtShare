...
function AuthForm({ mode }: { mode: AuthMode }) {
  ...
  const alternateHref = redirect
    ? `${copy.footerHref}?redirect=${encodeURIComponent(redirect)}`
    : copy.footerHref;
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      const storedDisplayName = userSnap.exists()
        ? String(userSnap.data().displayName || "").trim()
        : "";
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <Button
                  ...
                  disabled={loading}
                  variant="outline"
                  className="h-12 w-full rounded-2xl border-2 border-gray-200 bg-white text-sm font-bold text-gray-700 transition-all duration-300 hover:scale-[1.02] hover:border-gray-300 hover:bg-gray-50"
                >
                  {loading ? (
                    <div className="mr-3 h-4 w-4 animate-spin rounded-full border-2 border-emerald-600 border-t-transparent" />
                  ) : (
